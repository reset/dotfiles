...
|----------------|---------|--------------|
| `/opt/arr/disk-audit.py` | Honest disk accounting (hardlink-aware) + `--seed-status` per-torrent verdict. **Suppresses the orphan list when Transmission's view is unreliable** (RPC failed / disk critically full / torrents erroring) — see the "orphan cleanup while degraded" lesson. Also surfaces **staging dirs not owned by the transmission user** (root-owned write-blockers — see the "root-owned staging" lesson) | Manual |
| `/opt/arr/bulk-add.py` | Parse scene-named folders and bulk-add to Radarr/Sonarr | Manual |
| `/opt/arr/fix-seeding.py` | Restore hardlinks for files Sonarr/Radarr moved instead of hardlinked. Matches library copies by filename **and, as a fallback, by exact byte size** — so files the *arr renamed on import still relink (via `arrlib.choose_relink_source`). Chowns any staging dir it creates back to `111:113` so it doesn't reintroduce a root-owned write-blocker | Manual when seeding shows error |
| `/opt/arr/monitor.py` | Health check for Sonarr/Radarr/Prowlarr + Bazarr + Transmission **+ disk fullness (warn 95% / crit 98%), both of which email a rate-limited alert via Resend**. **Errored torrents now email too** (separate rate-limited stream — they used to only log, so a breakage could sit unseen for days), runs every 4h via `/etc/cron.d/arr-monitor` | Cron |
...
### Root-owned staging dirs → "Permission denied" torrent errors
A milder cousin of the above, seen 2026-08. A cleanup run **as root** swept sidecar junk (`.nfo`, `Sample-*.mp4`) out of the staging dirs and left ~38 folders owned by `root:root`. For most it was harmless — the deleted files weren't torrent members, and a root-owned 755 dir still lets Transmission (PUID 111 / PGID 113 = `debian-transmission`) **read** and keep seeding. But where a swept file *was* a torrent member, the torrent goes incomplete and Transmission tries to re-fetch it — and **can't write into the root-owned dir**, surfacing as `error 3: Couldn't get '…': Permission denied (13)`. No media is lost (the main `.mkv`/`.mp4` is hardlinked in the library, `nlink=2`). The tell: many staging dirs `root:root` with the *same* mtime (the cleanup's timestamp), while the files inside are still `debian-transmission`.

Remediation (recovers seeding, deletes nothing):
1. **Chown staging back**: `sudo find …/downloads/{radarr,tv-sonarr} -maxdepth 1 -type d -user root -exec chown -R 111:113 {} +`. Chowning a hardlinked staging file also chowns its library twin — harmless, `111:113` is the correct owner everywhere. `disk-audit.py` lists these dirs under "Staging dirs not owned by the transmission user".
2. **Relink whole-folder deletions** from the library (`fix-seeding.py` — now matches renamed files by size), or by hand with `os.link` if the *arr renamed it.
3. **stop → verify → start** the errored torrents. Transmission re-checks, sees the main file valid, and re-downloads the small missing sidecar from peers.
4. **A stuck torrent isn't necessarily dead**: one at 99% with `peers=0` looked dead, but `torrent-start-now` + `torrent-reannounce` woke a 399-seeder swarm and it finished in seconds. Kick it before concluding the swarm is gone.
